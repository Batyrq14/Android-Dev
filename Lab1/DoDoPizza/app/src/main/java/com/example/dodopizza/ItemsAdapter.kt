...
class ItemsAdapter(var items: List<Item>, var context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ...
            ITEM_TYPE_NEW_ITEM -> {
                val view = LayoutInflater.from(parent.context).inflate(R.layout.item_newpizza, parent, false)
                MyViewHolder2(view)
            }
    ...
    override fun getItemViewType(position: Int): Int {
        val item = items[position] // Assuming items is a list of Item objects
        return if (item.flag) {
            ITEM_TYPE_NEW_ITEM
        } else {
            ITEM_TYPE_ITEM
